...
#define RUN_TASKSTATS_ISZOMBIE(__stats) \
  (!__stats.hiwater_rss && !__stats.hiwater_vm && !__stats.ac_etime && !__stats.ac_utime && !__stats.ac_stime)
...
} run_process_limits_t; // Limits information
...
run_create_process                 (char *__cmd, char *__workdir, DWORD __rss_limit, DWORD __time_limit);
