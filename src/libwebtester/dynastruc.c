...
swap                               (dyna_item_t *__a, dyna_item_t *__b)
{
  ...
  stag  = __a->tag;  __a->tag=__b->tag;   __b->tag=stag;
  sdata = __a->data; __a->data=__b->data; __b->data=sdata;
}
...
{
  if (!__self) // Trying to work with wrong struct
    return -1;
...
dyna_delete                   (dynastruc_t *__self, dyna_item_t *__item, dyna_deleter __deleter)
{
  if (!__self) // Trying to work with wrong struct
    return -1;
  if (!__item) // Trying to delete zero item
    return -1;
...
dyna_push                     (dynastruc_t *__self, void *__data, int __tag)
{
  ...
  if (!new_ptr) // FATAL ERROR: Memory has not been allocated
    return -1;
  new_ptr->data = __data;
  new_ptr->tag = __tag;
  new_ptr->prev_ptr = 0;
...
dyna_pop                      (dynastruc_t *__self, void **__data, int *__tag)
{
  ...
  if (!__self) // Trying to work with wrong struct
    return -1;
  if (__self->count == 0) return -1;
  if (__data) *__data = __self->head->data;
  if (__tag) *__tag = __self->head->tag;
  temp_ptr = __self->head;
  __self->head = (dyna_item_t*)__self->head->next_ptr;
  dyna_delete (__self, temp_ptr, 0);
  if (!__self->head) __self->tail = 0;
  return 0;
}
...
dyna_search                   (dynastruc_t *__self, void *__data, int __tag, dyna_comparator __comparator)
{
  if (!__self) // Trying to work with wrong struct
    return 0;
  if (!__self->find_data)
    __self->find_data = __self->head; else
...
dyna_del_from_back            (dynastruc_t *__self, void **__data, int *__tag)
{
  ...
  if (!__self) // Trying to work with wrong struct
    return -1;
  if (__self->count == 0) return -1;
  if (__data) *__data = __self->tail->data;
  if (__tag) *__tag = __self->tail->tag;
