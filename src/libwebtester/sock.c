...
sock_create_inet                   (char *__host, unsigned int __port)
{
  ...
  if (setsockopt (sock, SOL_SOCKET, SO_REUSEADDR, (void *)&sock_opt, sizeof (sock_opt))<0) return -1;
...
sock_create_unix                   (char *__fn)
{
  ...
  if ((sock=socket (AF_UNIX, SOCK_STREAM, 0))==-1) return SE_CANT_CREATE_SOCK;
  local.sun_family=AF_UNIX;
  strcpy (local.sun_path, __fn);
  unlink(local.sun_path);
  len=strlen (local.sun_path)+sizeof (local.sun_family);
  if (bind (sock, (struct sockaddr *)&local, len)==-1) return -1;
  return sock;
}
...
sock_create_client_unix            (char *__fn)
{
  ...
  if (connect (sock, (struct sockaddr *)&remote, len)==-1) return -1;
  return sock;
}
