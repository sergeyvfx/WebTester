 ...
 *
 * ================================================================================
 *  uploader-checker.c - part of the WebTester Server
 * ================================================================================
 *
  ...
  if (!HTTP_STATUS_OK (*msg))
    {
      ...
      core_print (MSG_ERROR, "    Informatics: Error sending HTTP request for checker's list: %s\n", err);
...
save_checker                       (assarr_t *__params)
{
  ...
  char *ext=COMPILER_SAFE_PCHAR_KEY (compiler_id, "Extension", INFORMATICS_SRCEXT);
  char *src;
...
static BOOL
exec_compiler                      (assarr_t *__params, char *__cmd, char *__err)
{
  char *compiler_id=assarr_get_value (__params, "COMPILERID");
  char dir[4096];
  ...
  // Memory limit
  compiler_ml=COMPILER_SAFE_INT_KEY (compiler_id, "Limits/RSS", INFORMATICS_COMPILER_RSS_LIMIT);
  // Time limit
  compiler_tl=COMPILER_SAFE_FLOAT_KEY (compiler_id, "Limits/Time", INFORMATICS_COMPILER_TIME_LIMIT)*USEC_COUNT;
  ...
  if (RUN_PROC_EXEC_ERROR (*proc))
    {
      ...
      sprintf (__err, "Fatal error executing compiler: %s", RUN_PROC_ERROR_DESC (*proc));
  ...
  if (RUN_PROC_PIPEBUF (*proc))
      assarr_set_value (__params, "COMPILER_MESSAGES", strdup (RUN_PROC_PIPEBUF (*proc)));
  ...
  if (!fexists (fn))
    {
      ...
      if (!msg)
        strcpy (__err, "Compilation error"); else
...
static void
put_checker                        (assarr_t *__params, char *__err, char *__desc)
{
  ...
  sprintf (url, "%s&id=%s&err=%s&desc=%s", url_prefix, (char*)assarr_get_value (__params, "ID"), __err, desc);
  ...
  if (!HTTP_STATUS_OK (*msg))
    {
      core_print (MSG_ERROR, "    Informatics: Error sending HTTP request for return checker's uploading result: %d\n", HTTP_STATUS (*msg));
      http_message_free (msg);
    }
          ...
          if (check_params (params))
            {
              ...
              if (!save_checker (params))
                ...
                if (!compile (params, err))
                  put_checker (params, "CE", err); else
