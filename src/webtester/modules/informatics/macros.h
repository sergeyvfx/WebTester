...
#define INF_MESSAGE(__proc, __text, __args...) \
  __proc ("Informatics", __text, ##__args)

#define INF_INFO(__text, __args...) \
  INF_MESSAGE (MODULE_INFO,     __text, ##__args)

#define INF_WARNING(__text, __args...) \
  INF_MESSAGE (MODULE_WARNING,  __text, ##__args)

#define INF_ERROR(__text, __args...) \
  INF_MESSAGE (MODULE_ERROR,    __text, ##__args)

#define INF_DEBUG(__text, __args...) \
  INF_MESSAGE (MODULE_DEBUG,    __text, ##__args)

#define INF_KEY_ENTRY(__proc, __val, __path)  \
  __proc ("Informatics", __val, __path)

#define INF_INT_KEY(__val, __path) \
  INF_KEY_ENTRY (MODULE_INT_KEY,   __val, __path)

#define INF_FLOAT_KEY(__val, __path) \
  INF_KEY_ENTRY (MODULE_FLOAT_KEY, __val, __path)

#define INF_PCHAR_KEY(__val, __path) \
  INF_KEY_ENTRY (MODULE_PCHAR_KEY, __val, __path)
...
#define COMPILER_SAFE_VAL(__id,__path,__default_value,__func) \
  ((Informatics_compiler_config_val (__id, __path))? \
    (__func (Informatics_compiler_config_val (__id, __path))):(__default_value))
...
#define COMPILER_SAFE_COMMON_VAL(__path,__default_value,__func) \
  ((Informatics_compiler_common_val (__path))? \
    (__func (Informatics_compiler_common_val (__path))):(__default_value))
