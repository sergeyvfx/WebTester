...
read_config (void)
{
  CONFIG_FLOAT_KEY (queue_update_interval,
                    "Server/MainLoop/Queue/UpdateInterval");
  CONFIG_FLOAT_KEY (queue_autostart, "Server/MainLoop/Queue/Autostart");
  CONFIG_FLOAT_KEY (belts_update_interval,
                    "Server/MainLoop/Belts/UpdateInterval");
...
/**
 * Call tasks uploading process
 */
static void
call_upload_tasks (void)
{
  /* Call tasks upload hook */
  hook_call (CORE_UPLOADPROBLEMS, 0);
}
...
wt_mainloop (gpointer __unused)
{
  ...
  timeval_t last_queue_update, last_belts_update, cur_time, last_upload,
          last_unpack, last_scheduler;
  ...
  last_queue_update = last_belts_update = last_scheduler = last_unpack =
          last_upload = cur_time = now ();
  ...
  for (;;)
    {
      ...
      CALL_DELAYED (last_scheduler, cur_time, scheduler_delta,
                    scheduler_overview, 0);

      /* Unpack params of some tasks for some optimization */
      CALL_DELAYED (last_unpack, cur_time, unpack_delta, wt_queue_unpack, 0);

      /* Update queue */
      CALL_DELAYED (last_queue_update, cur_time, queue_delta,
                    wt_queue_update, check_state |= TRUE);

      /* Update belts */
      CALL_DELAYED (last_belts_update, cur_time, belts_delta,
                    wt_belts_update, check_state |= TRUE);
...
wt_mainloop_start (void *__unused, void *__call_unused)
{
  ...
  wt_reset_status ();
  core_print (MSG_INFO, "    **** Testing main loop started. "
                        "Queue size: %d, Belts size: %d.\n",
